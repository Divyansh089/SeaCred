...
export interface CarbonProject {
  ...
  createdAt: Date;
  updatedAt: Date;
  verificationStatus: number; // 0: PENDING, 1: IN_PROGRESS, 2: APPROVED, 3: REJECTED
...
export const getVerificationStatusText = (status: number | bigint): string => {
...
// Helper function to get verification status color classes
export const getVerificationStatusColor = (status: number | bigint): string => {
  const statusNumber = Number(status);
  switch (statusNumber) {
